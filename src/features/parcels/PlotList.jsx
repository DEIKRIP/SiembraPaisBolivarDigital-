...
const ParcelManagement = () => {
  ...
  useEffect(() => {
    const savedLanguage = localStorage.getItem('agroflow_language') || 'es';
  ...
  const handleAddParcel = (newParcel) => {
    // Crear un nuevo objeto de parcela con un ID único
    const parcelWithId = {
      ...newParcel,
      id: `P-${new Date().getFullYear()}-${String(parcels.length + 1).padStart(3, '0')}`,
      farmerName: farmers.find(f => f.id === newParcel.farmerId)?.name || 'Desconocido',
      createdAt: new Date().toISOString(),
      lastInspection: null
    };
    
    // Actualizar el estado de las parcelas
    const updatedParcels = [parcelWithId, ...parcels];
    setParcels(updatedParcels);
